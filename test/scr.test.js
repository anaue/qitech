...
describe('GET SCR', function () {
  const mockedData = mockedReqRes.scr.get;

  let scr_key = mockedData.request;
  const decoder = () => mockedData.status_200;
  beforeEach(() => {
    const scope = nock('https://api-auth.sandbox.qitech.app')
      .get(util.format(qitech.scr.RESOURCE_PATH, scr_key))
      .reply(200,{});
  });
  ...
  it('gets a scr', () => {
    ...
    return qitech.scr.get(scr_key, options).then(response => {
      expect(typeof response).to.equal('object');
      expect(response.data).not.to.equal(null);
...
describe('POST REDO SCR', function () {
    const mockedData = mockedReqRes.scr.post_redo;
    const decoder = () => mockedData.status_200;
    beforeEach(() => {
      const scope = nock('https://api-auth.sandbox.qitech.app')
    ...
    it('redo SCR', () => {
      ...
      let data = mockedData.request;
      return qitech.scr.post_redo(data, options).then(response => {
        expect(typeof response).to.equal('object');
        expect(response.data).not.to.equal(null);
        expect(response.data.webhook_type).to.equal(mockedData.status_200.webhook_type);
        expect(response.data.key).to.equal(mockedData.status_200.key);
