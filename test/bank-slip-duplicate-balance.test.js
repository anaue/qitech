...
describe('Daily Duplicate Balance', function () {
...
describe('Daily Excel Duplicate Balance', function () {
  const mockedData = mockedReqRes.bank_slip_duplicate_balance.get_excel;
  const decoder = () => mockedData.status_200;
  beforeEach(() => {
    const scope = nock('https://api-auth.sandbox.qitech.app')
  ...
  it('get Daily Excel Duplicate Balance', () => {
    ...
    let data = mockedData.request;
    return qitech.bank_slip_duplicate_balance.get_excel(data, options).then(response => {
      expect(typeof response).to.equal('object');
